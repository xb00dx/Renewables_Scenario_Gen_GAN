# model adapted for tensorflow version 2
# borrowed from: https://github.com/Bittencourt/Renewables_Scenario_Gen_GAN
...
def batchnormalize(X, eps=1e-8, g=None, b=None):
    if X.get_shape().ndims == 4:
        mean = tf.reduce_mean(input_tensor=X, axis=[0,1,2])
        std = tf.reduce_mean( input_tensor=tf.square(X-mean), axis=[0,1,2] )
        ...
        mean = tf.reduce_mean(input_tensor=X, axis=0)
        std = tf.reduce_mean(input_tensor=tf.square(X-mean), axis=0)
        X = (X-mean) / tf.sqrt(std+eps)
...
def bce(o, t):
    ...
    return tf.reduce_mean(input_tensor=tf.nn.sigmoid_cross_entropy_with_logits(o, t))
...
class GAN():
    ...
    def build_model(self):
        ...
        Z = tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.dim_z])
        Y = tf.compat.v1.placeholder(tf.float32, [self.batch_size, self.dim_y])

        image_real = tf.compat.v1.placeholder(tf.float32, [self.batch_size]+self.image_shape)
        ...
        p_real=tf.reduce_mean(input_tensor=raw_real2)

        raw_gen2 = self.discriminate(image_gen, Y)
        #p_gen = tf.nn.sigmoid(raw_gen)
        p_gen = tf.reduce_mean(input_tensor=raw_gen2)

        discrim_cost = tf.reduce_sum(input_tensor=raw_real2) - tf.reduce_sum(input_tensor=raw_gen2)
        gen_cost = -tf.reduce_mean(input_tensor=raw_gen2)
    ...
    def samples_generator(self, batch_size):
        Z = tf.compat.v1.placeholder(tf.float32, [batch_size, self.dim_z])
        Y = tf.compat.v1.placeholder(tf.float32, [batch_size, self.dim_y])
